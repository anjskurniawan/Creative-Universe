...
export function Header({
  ...
  const handleAddProject = (e: React.MouseEvent) => {
    ...
    if (onAddProject) {
      ...
      alert("Buat Proyek Baru...");
    }
  };

  // Render saat mode collapsed (Hanya icon Folder dengan Tooltip "Proyek Saya")
  if (!isExpanded) {
    return (
      <TooltipTrigger placement="right">
        ...
        >
          <IconSpectrum name="Folder" styles={iconStyle({ size: "L" })} />
        </AriaButton>
        <Tooltip>Proyek Saya</Tooltip>
      </TooltipTrigger>
    );
  }
  ...
  return (
    <div className="flex items-center justify-between px-3 py-1.5 mt-2">
      ...
      {/* Tombol Aksi Tambah Proyek / Folder di Kanan Header */}
      <TooltipTrigger placement="top">
        <button
          type="button"
          aria-label="Tambah Folder Proyek"
          onClick={handleAddProject}
          className="flex h-6 w-6 items-center justify-center rounded-md text-white/50 hover:bg-white/10 hover:text-white transition-colors"
        >
          <IconSpectrum
            name="FolderAdd"
            styles={iconStyle({ size: "S" })}
        ...
        <Tooltip>Tambah Folder Proyek</Tooltip>
      </TooltipTrigger>
    </div>
  );
}
