...
export type PerformanceSidebarItem = {
  ...
  href?: string;
  badge?: number | string;
  status?: "Default" | "Hover" | "Active" | "Highlight";
  group?: string;
  isActive?: boolean;
...
function SidebarLink({ href, icon, label, active = false, highlight = false, theme, expanded, badge }: { href: string; icon: string; label: string; active?: boolean; highlight?: boolean; theme: "dark" | "light" | "retro"; expanded: boolean; badge?: number | string }) {
  ...
  return <Link href={href} aria-label={label} aria-current={active ? "page" : undefined} className={`flex h-8 items-center rounded-lg border-2 border-transparent transition focus-visible:outline-none focus-visible:ring-2 ${expanded ? "w-full gap-3 px-2" : "w-8 justify-center"} ${colorClasses}`}>
...
/** Reusable shell sidebar for performance/system layouts. */
export function PerformanceSidebar({ theme, onToggleTheme, onToggleRetro, expanded, onToggleExpanded, activeHref = "/kv-retail/performance", ariaLabel = KV_RETAIL_PERFORMANCE_PAGE.title, className = "", primaryItems = PRIMARY_ITEMS, settingsHref = "/kv-retail/option" }: { theme: "dark" | "light" | "retro"; onToggleTheme: () => void; onToggleRetro: () => void; expanded: boolean; onToggleExpanded: () => void; activeHref?: string; ariaLabel?: string; className?: string; primaryItems?: readonly PerformanceSidebarItem[] | PerformanceSidebarItem[]; settingsHref?: string }) {
  const light = theme !== "dark";
        ...
        {primaryItems.map((item, index) => {
          ...
          return (
            <Fragment key={item.label}>
              ...
              <SidebarLink href={item.href ?? "#"} icon={item.icon} label={item.label} active={item.isActive ?? (activeHref ? item.href === activeHref : false)} highlight={item.isHighlighted} theme={theme} expanded={expanded} badge={item.badge} />
            </Fragment>
          );
        })}
      </div>
