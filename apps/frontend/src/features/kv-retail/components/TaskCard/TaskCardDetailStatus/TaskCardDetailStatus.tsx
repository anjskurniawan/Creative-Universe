...
export default function TaskCardDetailStatus({
  ...
  const iconName = !hasAnyFile
    ? (config.icon_file_empty || "expand_more")
    : (config.icon_file_filled || "expand_more");
  ...
  return (
    <div className={`relative ${className}`} ref={dropdownRef}>
      ...
      >
        <p
          className={[
            "min-w-0 flex-1 truncate whitespace-nowrap text-[12px] font-normal leading-normal font-['Google_Sans_Flex']",
            textClasses
          ].join(" ")}
          style={textStyle}
        >
        ...
        <div className="relative flex size-[18px] shrink-0 items-center justify-center">
          <MaterialIcon
            name={isOpen ? "expand_less" : iconName}
            size="auto"
            weight={400}
            filled={false}
            className={["block !text-[18px] leading-none", textClasses].join(" ")}
      ...
      {isOpen && (
        <div className={`absolute top-full left-0 z-50 mt-1 flex w-[130px] flex-col overflow-hidden rounded-[8px] shadow-sm ${theme === "dark" ? "border border-white/10 bg-[#202820]" : theme === "retro" ? "border-2 border-[#24252b] bg-[#eceee6]" : "border border-cu-line bg-white"}`}>
          {files.map((file, idx) => (
            ...
            >
              <MaterialIcon 
                name={file ? "visibility" : "upload_file"} 
                size="auto" 
                weight={400}
                filled={false}
                className="shrink-0 !text-[18px] leading-none text-cu-muted"
                style={{ fontSize: "18px" }}
              />
