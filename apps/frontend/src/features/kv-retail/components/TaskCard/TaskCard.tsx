...
import TaskCardDate from "./TaskCardDate/TaskCardDate";
import TaskCardNextButton from "./TaskCardNextButton/TaskCardNextButton";
import TaskCardTitleTask from "./TaskCardTitleTask/TaskCardTitleTask";
import TaskCardLoadingBar from "./TaskCardLoadingBar/TaskCardLoadingBar";
import TaskCardDetailStatus from "./TaskCardDetailStatus/TaskCardDetailStatus";
import TaskCardDetail from "./TaskCardDetail/TaskCardDetail";
import TaskCardButtonStatus from "./TaskCardButtonStatus/TaskCardButtonStatus";
import TaskCardDeleteOverlay from "./TaskCardDeleteOverlay/TaskCardDeleteOverlay";
import TaskCardSubmitLinkOverlay from "./TaskCardSubmitLinkOverlay/TaskCardSubmitLinkOverlay";
import TaskCardViewLinkOverlay from "./TaskCardViewLinkOverlay/TaskCardViewLinkOverlay";
import TaskCardUploadOverlay from "./TaskCardUploadOverlay/TaskCardUploadOverlay";
import TaskCardDelayReasonOverlay from "./TaskCardDelayReasonOverlay/TaskCardDelayReasonOverlay";
import { TaskcardMobileLayoutCard, type TaskcardMobileAvatar, type TaskcardMobileChange } from "./TaskCardMobile";
import { resolveStorageUrl } from "@/core/api/client";
import { kvRetailApi } from "@/features/kv-retail/api";
import { MaterialIcon } from "@/components/ui/MaterialIcon/MaterialIcon";
import type { TaskCardButtonStatusState, TaskCardButtonStatusType } from "./TaskCardButtonStatus/TaskCardButtonStatus";
import type { TaskCardNextButtonState } from "./TaskCardNextButton/TaskCardNextButton";
import type { TaskCardDetailStatusState } from "./TaskCardDetailStatus/TaskCardDetailStatus";
...
export type TaskCardProps = {
  ...
  onDeadlineSave?: (deadline: string) => Promise<void>;
...
export default function TaskCard({ 
  ...
  return (
    <div ref={cardShellRef} className={`cu-style relative w-full rounded-2xl hover:z-10 focus-within:z-10 ${theme === "dark" ? "border border-white/10 bg-[#171717] shadow-[0_10px_26px_rgba(0,0,0,0.22)]" : theme === "retro" ? "border-2 border-[#24252b] bg-[#eceee6] shadow-[3px_3px_0_#24252b]" : "border border-[#e5e7eb] bg-white shadow-sm"}`}>
      {/* Main Card Content */}
          ...
          >
            ...
            <div className={`flex ${isCompactDesktop ? "min-w-0 flex-1 items-center gap-2" : "flex-col sm:flex-row gap-4 xl:gap-4 2xl:gap-6 items-stretch sm:items-center relative min-w-0 xl:flex-none xl:mr-6"}`}>
              ...
              <div className={`${isCompactDesktop ? "hidden" : "flex"} flex-col gap-[7px] items-start relative shrink-0 mt-3 sm:mt-0 xl:w-[130px]`}>
                ...
                <TaskCardDetail variant="Date" value={formattedDeadline} deadlineValue={deadlineDate} editable={canManageTask && Boolean(id) && !isDone} onSave={onDeadlineSave} config={config} theme={theme} />
                {!isDone && (
