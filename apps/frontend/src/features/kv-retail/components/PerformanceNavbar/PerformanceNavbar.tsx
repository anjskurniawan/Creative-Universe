...
export function PerformanceNavbar({
  ...
  const { renderMessageBell, renderNotificationBell } = useCommunicationActions();
  ...
  return <nav className={`flex h-16 shrink-0 items-center justify-between ${compact ? "px-4" : "px-5"} ${retro ? "border-b-[3px] border-[#24252b] bg-[#eceee6]" : theme === "light" ? "border-b border-black/[0.045] bg-white/55" : "border-b border-white/[0.06] bg-[#111413]/55"}`} aria-label={`Navigasi ${title}`} data-kv-retail-navbar data-compact={compact || undefined}>
    ...
    <div className="flex items-center gap-1">
      ...
      {user ? renderMessageBell({ userId: user.id, variant: light ? "light" : "dark", panelClassName: bellPanelClass, renderTrigger: ({ toggle }) => <IconButton icon="chat_bubble" label="Pesan" onClick={toggle} theme={theme} /> }) : <IconButton icon="chat_bubble" label="Pesan" onClick={() => router.push(APP_ROUTES.messages)} theme={theme} />}
      {user ? renderNotificationBell({ userId: user.id, variant: light ? "light" : "dark", panelClassName: bellPanelClass, renderTrigger: ({ toggle }) => <IconButton icon="notifications" label="Notifikasi" onClick={toggle} theme={theme} /> }) : <IconButton icon="notifications" label="Notifikasi" onClick={() => router.push(APP_ROUTES.notifications)} theme={theme} />}
