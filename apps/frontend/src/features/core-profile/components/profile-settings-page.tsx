...
function settingBoolean(user: User, key: string, fallback: boolean): boolean {
  const value = user.settings?.[key];
...
function PreferenceToggle({ label, description, checked, onChange }: { label: string; description: string; checked: boolean; onChange: (checked: boolean) => void }) {
  return (
    <label className="flex cursor-pointer items-center justify-between gap-4 rounded-xl border border-cu-line bg-cu-panel-soft/50 p-4">
      <span><span className="block text-sm font-medium text-cu-ink">{label}</span><span className="mt-1 block text-xs text-cu-muted">{description}</span></span>
      <input type="checkbox" checked={checked} onChange={(event) => onChange(event.target.checked)} className="size-4 shrink-0 rounded border-cu-line text-cu-ink" />
    </label>
  );
}
...
export default function ProfileSettingsPage() {
  ...
  const activeTab = (searchParams.get("tab") || mobileRouteTab[normalizedPathname] || "profile") as
    "profile" | "notifications" | "privacy" | "applications" | "security" | "role_settings" | "activity_log" |
  ...
  const [notificationTaskUpdates, setNotificationTaskUpdates] = useState(true);
  const [notificationMentions, setNotificationMentions] = useState(true);
  const [notificationDeadlines, setNotificationDeadlines] = useState(true);
  const [quietStart, setQuietStart] = useState("");
  const [quietEnd, setQuietEnd] = useState("");
  const [profileShowApplications, setProfileShowApplications] = useState(true);
  ...
  useEffect(() => {
    ...
    if (user) queueMicrotask(() => {
      ...
      setHighContrast(settingBoolean(user, "high_contrast", false));
      setNotificationInApp(settingBoolean(user, "notification_in_app", true));
      setNotificationWhatsapp(settingBoolean(user, "notification_whatsapp", false));
      setNotificationTaskUpdates(settingBoolean(user, "notification_task_updates", true));
      setNotificationMentions(settingBoolean(user, "notification_mentions", true));
      setNotificationDeadlines(settingBoolean(user, "notification_deadlines", true));
      setQuietStart(settingValue(user, "notification_quiet_start", ""));
      setQuietEnd(settingValue(user, "notification_quiet_end", ""));
      setProfileShowApplications(settingBoolean(user, "profile_show_applications", true));
  ...
  const savePreferenceSettings = async (settings: Record<string, unknown>, successMessage: string) => {
    ...
    try {
      await apiFetch<User>("/profile", {
        method: "PATCH",
        body: JSON.stringify({ name, username, email: email || null, whatsapp_number: whatsappNumber || null, settings }),
      });
      await refreshUser();
      setPreferencesStatus(successMessage);
    } catch (error) {
      setPreferencesError(validationMessage(error));
    } finally {
      ...
      <div className="animate-fade-in">
        ...
        {activeTab === "profile" && (
              ...
              <div className="flex-1 w-full space-y-4 max-w-xl">
                ...
                <div className="pt-4 border-t border-cu-line/60 space-y-4">
                  ...
                  <div className="grid gap-4 sm:grid-cols-2">
                    <div>
                      <label className="block text-sm font-semibold text-cu-ink" htmlFor="language-select">Bahasa</label>
                      <select id="language-select" value={language} onChange={(event) => setLanguage(event.target.value)} className="mt-1.5 block w-full rounded-lg border border-cu-line bg-cu-surface px-3 py-2 text-sm text-cu-ink">
                        <option value="id">Bahasa Indonesia</option>
                        <option value="en">English</option>
                      </select>
                    </div>
                    <div>
                      <label className="block text-sm font-semibold text-cu-ink" htmlFor="timezone-select">Zona waktu</label>
                      <select id="timezone-select" value={timezone} onChange={(event) => setTimezone(event.target.value)} className="mt-1.5 block w-full rounded-lg border border-cu-line bg-cu-surface px-3 py-2 text-sm text-cu-ink">
                        <option value="Asia/Bangkok">WIB (UTC+7)</option>
                  ...
                  <label className="flex cursor-pointer items-center justify-between gap-4 rounded-xl border border-cu-line bg-cu-surface p-4">
                    <span><span className="block text-sm font-semibold text-cu-ink">Kurangi animasi</span><span className="mt-1 block text-xs text-cu-muted">Simpan preferensi untuk antarmuka yang lebih tenang.</span></span>
                    <input type="checkbox" checked={reduceMotion} onChange={(event) => setReduceMotion(event.target.checked)} className="size-4 rounded border-cu-line text-cu-ink" />
                  </label>
                  <label className="flex cursor-pointer items-center justify-between gap-4 rounded-xl border border-cu-line bg-cu-surface p-4">
                    <span><span className="block text-sm font-semibold text-cu-ink">Kontras tinggi</span><span className="mt-1 block text-xs text-cu-muted">Simpan preferensi aksesibilitas untuk penerapan berikutnya.</span></span>
                    <input type="checkbox" checked={highContrast} onChange={(event) => setHighContrast(event.target.checked)} className="size-4 rounded border-cu-line text-cu-ink" />
                  </label>
          ...
          {activeTab === "notifications" && (
            <form onSubmit={(event) => { event.preventDefault(); void savePreferenceSettings({ notification_in_app: notificationInApp, notification_whatsapp: notificationWhatsapp, notification_task_updates: notificationTaskUpdates, notification_mentions: notificationMentions, notification_deadlines: notificationDeadlines, notification_quiet_start: quietStart || null, notification_quiet_end: quietEnd || null }, "Preferensi notifikasi berhasil disimpan."); }} className="space-y-6 animate-fade-in">
              <div className="border-b border-cu-line pb-3"><h2 className="text-2xl font-semibold text-cu-ink">Notifikasi</h2><p className="mt-1 text-sm text-cu-muted">Preferensi ini disimpan terlebih dahulu. Pengiriman WhatsApp per-event akan diaktifkan saat integrasinya siap.</p></div>
              {preferencesStatus && <p className="rounded-lg border border-cu-success/20 bg-cu-success-soft p-4 text-sm text-cu-success">{preferencesStatus}</p>}
              {preferencesError && <p className="rounded-lg border border-cu-danger/20 bg-cu-danger-soft p-4 text-sm text-cu-danger">{preferencesError}</p>}
              <div className="space-y-3 rounded-2xl border border-cu-line bg-cu-surface p-5">
                <PreferenceToggle label="Notifikasi dalam aplikasi" description="Tampilkan notifikasi pada Creative Universe." checked={notificationInApp} onChange={setNotificationInApp} />
                <PreferenceToggle label="Notifikasi WhatsApp" description="Simpan preferensi kanal WhatsApp untuk integrasi berikutnya." checked={notificationWhatsapp} onChange={setNotificationWhatsapp} />
              </div>
              <div className="space-y-3 rounded-2xl border border-cu-line bg-cu-surface p-5"><h3 className="text-sm font-semibold text-cu-ink">Peristiwa yang ingin diterima</h3><PreferenceToggle label="Update tugas" description="Task baru, perubahan status, dan assignment." checked={notificationTaskUpdates} onChange={setNotificationTaskUpdates} /><PreferenceToggle label="Mention dan pesan" description="Saat Anda disebut atau menerima aktivitas percakapan." checked={notificationMentions} onChange={setNotificationMentions} /><PreferenceToggle label="Deadline" description="Pengingat tenggat waktu dan keterlambatan." checked={notificationDeadlines} onChange={setNotificationDeadlines} /></div>
              <div className="rounded-2xl border border-cu-line bg-cu-surface p-5"><h3 className="text-sm font-semibold text-cu-ink">Jam hening</h3><p className="mt-1 text-xs text-cu-muted">Disimpan sebagai preferensi untuk aturan pengiriman mendatang.</p><div className="mt-4 grid gap-4 sm:grid-cols-2"><label className="text-sm font-medium text-cu-ink">Mulai<input type="time" value={quietStart} onChange={(event) => setQuietStart(event.target.value)} className="mt-1.5 block w-full rounded-lg border border-cu-line bg-cu-surface px-3 py-2" /></label><label className="text-sm font-medium text-cu-ink">Selesai<input type="time" value={quietEnd} onChange={(event) => setQuietEnd(event.target.value)} className="mt-1.5 block w-full rounded-lg border border-cu-line bg-cu-surface px-3 py-2" /></label></div></div>
              <button type="submit" disabled={isSavingPreferences} className="inline-flex h-10 items-center justify-center rounded-full bg-cu-ink px-5 text-sm font-medium text-cu-surface disabled:opacity-50">{isSavingPreferences ? "Menyimpan..." : "Simpan Preferensi"}</button>
            </form>
          )}

          {activeTab === "privacy" && (
            <form onSubmit={(event) => { event.preventDefault(); void savePreferenceSettings({ profile_show_applications: profileShowApplications }, "Preferensi privasi berhasil disimpan."); }} className="space-y-6 animate-fade-in">
              <div className="border-b border-cu-line pb-3"><h2 className="text-2xl font-semibold text-cu-ink">Privasi Profil</h2><p className="mt-1 text-sm text-cu-muted">Atur informasi profil yang boleh ditampilkan kepada pengguna lain.</p></div>
              {preferencesStatus && <p className="rounded-lg border border-cu-success/20 bg-cu-success-soft p-4 text-sm text-cu-success">{preferencesStatus}</p>}
              {preferencesError && <p className="rounded-lg border border-cu-danger/20 bg-cu-danger-soft p-4 text-sm text-cu-danger">{preferencesError}</p>}
              <div className="rounded-2xl border border-cu-line bg-cu-surface p-5"><PreferenceToggle label="Tampilkan daftar aplikasi saya" description="Izinkan daftar aplikasi yang Anda akses terlihat pada profil pengguna lain." checked={profileShowApplications} onChange={setProfileShowApplications} /></div>
              <button type="submit" disabled={isSavingPreferences} className="inline-flex h-10 items-center justify-center rounded-full bg-cu-ink px-5 text-sm font-medium text-cu-surface disabled:opacity-50">{isSavingPreferences ? "Menyimpan..." : "Simpan Privasi"}</button>
            </form>
          )}

          {activeTab === "applications" && (
            <div className="space-y-6 animate-fade-in"><div className="border-b border-cu-line pb-3"><h2 className="text-2xl font-semibold text-cu-ink">Aplikasi Saya</h2><p className="mt-1 text-sm text-cu-muted">Akses aplikasi dikelola administrator. Landing setelah login tetap berada di halaman utama.</p></div><div className="grid gap-3 sm:grid-cols-2">{user.applications.filter((application) => application.type === "sub_app").sort((left, right) => left.sort_order - right.sort_order).map((application) => <div key={application.key} className="rounded-2xl border border-cu-line bg-cu-surface p-5"><MaterialIcon name="apps" size="sm" className="text-cu-muted" /><h3 className="mt-3 text-sm font-semibold text-cu-ink">{application.display_name}</h3><p className="mt-1 text-xs text-cu-muted">{application.status === "experimental" ? "Eksperimen" : "Aktif"}</p></div>)}{user.applications.filter((application) => application.type === "sub_app").length === 0 && <p className="text-sm text-cu-muted">Belum ada aplikasi yang dapat diakses.</p>}</div></div>
          )}
