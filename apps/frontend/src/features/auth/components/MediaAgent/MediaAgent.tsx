import type { MediaAgentProps } from "./MediaAgent.types";

export type { MediaAgentProps } from "./MediaAgent.types";

export function MediaAgent({ src, alt = "Creative agent" }: MediaAgentProps) {
  const isVideo = Boolean(src && /\.(mp4|webm|ogg)(?:\?.*)?$/i.test(src));

  return (
    <div className="cu-style media-agent-enter flex min-h-0 h-full w-full items-end justify-center overflow-hidden">
      {src ? (
        isVideo ? (
