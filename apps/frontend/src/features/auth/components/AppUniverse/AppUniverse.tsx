...
export function ApplicationUniverse({ applications, isReady, className = "", isExiting = false }: AppUniverseProps) {
  ...
  return (
    <section aria-label="Aplikasi yang tersedia di Creative Universe" className={`cu-style relative isolate flex min-h-[28rem] items-center justify-center overflow-hidden px-6 py-8 lg:min-h-0 ${className}`}>
          ...
          return (
            <div key={application.key} data-orbit={config.orbit} className="pointer-events-none absolute inset-0 will-change-transform">
              <div data-planet className={`absolute ${config.position} -translate-x-1/2 -translate-y-1/2`}>
                ...
                >
                  <IconMaterial name={APPLICATION_ICONS[application.key] ?? "apps"} size="auto" className="text-[clamp(0.95rem,2.5vw,1.45rem)]" />
