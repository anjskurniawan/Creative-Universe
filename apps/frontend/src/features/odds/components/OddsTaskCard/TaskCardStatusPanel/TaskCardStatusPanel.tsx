...
import { RecommendationButton } from "@/features/odds/components/OddsTaskCard/RecommendationButton/RecommendationButton";
...
export function TaskCardWideStatusPanel({ isDone, isReview, status, statusDescription, feedbackHref, highlightLabel, highlightValue, rating, timerText, onRecommendation, recommendationDisabled = false }: WideStatusProps) {
  ...
  return (
    <div className={`flex w-[160px] shrink-0 flex-col justify-center gap-2 px-2 py-2 text-white ${isDone ? "bg-[#17633d]" : "bg-[#0077bf]"}`}>
      {highlightLabel ? (
        <RecommendationButton label={highlightLabel} onClick={onRecommendation} disabled={recommendationDisabled} />
      ) : showsRating ? (
        <div className="flex h-[36px] items-center gap-1.5" aria-label={rating != null ? `Rating ${rating} dari 5` : "Belum ada rating"}>
          ...
          {rating != null && !isDone && <span className="text-xs font-semibold text-white/90">{rating}/5</span>}
        </div>
      ) : highlightValue ? (
        <span className="flex h-[36px] items-center font-mono text-[26px] font-bold leading-none tracking-tight">{highlightValue}</span>
      ) : (
        <span className="flex h-[36px] items-center font-mono text-[26px] font-bold leading-none tracking-tight">{timerText}</span>
      )}
      <div className="flex items-center gap-1.5 border-t border-white/30 pt-1.5">
        <div className="min-w-0">
