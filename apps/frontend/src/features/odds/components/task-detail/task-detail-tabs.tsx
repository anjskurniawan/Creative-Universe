...
export function TaskDetailTabs({ activeTab, onChange, mobileOpen, onMobileOpenChange, navClass, tabButtonClass, className = "", "data-qa-component": qaComponent }: TaskDetailTabsProps) {
  ...
  return <>
    ...
    <nav className={`${navClass} ${className} hidden min-h-11 w-full shrink-0 lg:flex`} data-qa-component={qaComponent} aria-label="Detail task">{tabs.map(([tab, icon, label]) => <button key={tab} type="button" onClick={() => onChange(tab)} className={`inline-flex h-9 shrink-0 items-center justify-center gap-2 rounded-t-xl rounded-b-none px-5 text-sm font-semibold transition-all duration-200 ${tab === "info" ? "lg:hidden" : ""} ${tabButtonClass(tab)}`}><MaterialIcon name={icon} size="sm" style={{ fontSize: "20px" }} />{label}</button>)}</nav>
  </>;
}
