...
export function ClientTableBriefEditor({ task, briefText, theme, returnNote, onChange }: { task: OddsTask; briefText: string; theme: "light" | "dark" | "retro"; returnNote: string; onChange: (value: string) => void }) {
  ...
  return <div className="grid h-full min-h-0 min-w-0 grid-cols-1 gap-4 overflow-hidden lg:grid-cols-4"><div className="min-h-0 min-w-0 overflow-y-auto lg:col-span-3"><TableBriefDetails category={category} product={product} packagingImageName="" packagingImageId={null} rows={rows} uploadingPackagingImage={false} onCategoryChange={setCategory} onProductChange={setProduct} onPackagingImageUpload={() => undefined} onRowChange={(id, field, value) => sync(rows.map((row) => row.id === id ? { ...row, [field]: value } : row))} onIllustrationUpload={upload} uploadingIllustrationId={uploadingIllustrationId} onAddRow={() => sync([...rows, { id: `revision-${rows.length + 1}`, image_order: String(rows.length + 1), image_description: "", image_illustration: "", image_illustration_id: null, additional_notes: "" }])} onRemoveRow={(id) => sync(rows.filter((row) => row.id !== id))} onReorderRows={() => undefined} dark={theme === "dark"} form={form} update={() => undefined} selectedCategory={undefined} todayDate="" tomorrowDate="" threeDaysDate="" hideDetailInputs /></div><aside className={`min-h-0 overflow-y-auto rounded-xl border p-4 ${theme === "dark" ? "border-white/10 bg-white/5 text-white" : "border-cu-border bg-cu-panel-soft text-cu-ink"}`}><div className="mb-2 flex items-center gap-2 text-sm font-semibold"><MaterialIcon name="keyboard_return" size="sm" />Keterangan Brief Return</div><p className="whitespace-pre-wrap text-sm leading-6 text-cu-muted">{returnNote || "Belum ada keterangan return dari designer."}</p></aside></div>;
}
