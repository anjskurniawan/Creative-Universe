...
import { TaskFeedbackToast, TaskFeedbackToastHost } from "@/features/odds/components/OddsTaskCard";
import { OddsThemeContext } from "@/features/odds/context/OddsThemeContext";
...
export function OddsShell({ children }: { children: ReactNode }) {
  ...
  return (
    <OddsThemeContext.Provider value={{ theme: desktopTheme, setTheme: setDesktopTheme }}>
      ...
      <div className={`cu-style h-screen w-screen overflow-hidden ${desktopTheme === "dark" ? "bg-[#111413]" : desktopTheme === "retro" ? "bg-[#dfe2d3]" : "bg-[radial-gradient(circle_at_8%_6%,#00e7ef_0,transparent_25%),radial-gradient(circle_at_95%_90%,#00a4ff_0,transparent_31%),linear-gradient(135deg,#00a4ff_0%,#000675_44%,#04044a_100%)]"}`}>
        <div id="odds-shell-modal-root" className="h-full">
          <Container
            viewport={viewport}
            contentProps={{
              className: `flex h-full w-full flex-col overflow-hidden rounded-none shadow-[0px_14px_42px_0px_rgba(44,42,39,0.16)] lg:rounded-[16px] ${desktopTheme === "dark" ? "bg-[#111413] text-white" : desktopTheme === "retro" ? "bg-[#c9ccc0] font-mono" : "bg-[#f3fbff]"}`,
