...
export interface OddsTaskSkipRequest {
  id: number;
  task_id: number;
  designer_id: number;
  reason: string;
...
export async function getOddsAssignableUsers(): Promise<OddsAssignableUser[]> {
  const [designers, videographers, contentCreators, spvs] = await Promise.all([
    ...
    apiFetch<OddsPagination<OddsAssignableUser>>("/users?role=Content Creator&per_page=50"),
  ...
  const users = [
    ...
    ...normalizePage(contentCreators).data,
...
export async function requestOddsQueueSkip(id: string | number, reason: string): Promise<OddsTaskSkipRequest> {
...
export async function reviewOddsQueueSkip(
  id: string | number,
  decision: "approved" | "rejected",
  note?: string
): Promise<OddsTaskSkipRequest> {
  return apiFetch<OddsTaskSkipRequest>(`/odds/skip-requests/${id}/review`, {
    method: "POST",
    body: JSON.stringify({ decision, note }),
  });
}

export async function extendOddsTaskDeadline(
  id: string | number,
  deadline: string,
  note?: string
): Promise<OddsTask> {
  return apiFetch<OddsTask>(`/odds/tasks/${id}/extend-deadline`, {
    method: "POST",
