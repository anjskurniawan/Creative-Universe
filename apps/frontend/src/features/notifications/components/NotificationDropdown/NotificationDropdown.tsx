  ...
  return (
    ...
    >
        ...
        <Link
          ...
          onClick={onClose}
          className="shrink-0 text-[10px] font-semibold text-[#5b7190] transition-colors hover:text-[#04044a]"
        >
          Lihat Semua
        </Link>
      </div>
          ...
          notifications.map((notification) => (
            <li key={notification.id}>
              <button
                type="button"
                onClick={onClose}
                className={`flex w-full items-start gap-2.5 rounded-xl px-2.5 py-2 text-left transition-colors hover:bg-slate-50 ${!notification.read ? "bg-slate-50" : ""}`}
              >
