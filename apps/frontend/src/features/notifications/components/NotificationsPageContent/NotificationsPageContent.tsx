...
function NotificationListItem({ notification }: { notification: NotificationItem }) {
  ...
  return notification.url ? <Link key={notification.id} href={notification.url} className={className}>{content}</Link> : <div className={className}>{content}</div>;
}
  ...
  return (
    <div className="mx-auto w-full max-w-4xl py-4 md:py-8">
      ...
      <section className="mt-6 overflow-hidden rounded-2xl border border-cu-line bg-cu-surface shadow-sm">
        ...
        {!isLoading && notifications.map((notification) => <NotificationListItem key={notification.id} notification={notification} />)}
      </section>
    </div>
  );
}
