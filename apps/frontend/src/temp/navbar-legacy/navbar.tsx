  ...
  return (
    <nav className={`${sticky ? "sticky top-0" : "relative"} isolate z-[100] h-[72px] ${navClass}`}>
      <div className="flex h-full w-full items-center justify-between px-6 md:px-16">
        <Link href={APP_ROUTES.home} className="inline-flex size-9 shrink-0 items-center justify-center" aria-label="Creative Universe">
        ...
        {isAuthenticated && user && (
          <div className="flex items-center justify-end gap-2">
            {isRootUser && (
              <ActionLink href={APP_ROUTES.maintenance} label="Developer Panel" icon="code" className={actionClass} />
            )}
  ...
  return (
    <li>
      <Link href={href} onClick={onClick} className={className} role="menuitem">
        ...
        {badge && <span className="ml-auto rounded-full bg-cu-panel-soft px-2 py-0.5 text-[10px] text-cu-muted">{badge}</span>}
      </Link>
    </li>
  );
}
  ...
  return (
    <div className={profileDropdownPanelClass(isDark)}>
      ...
      <ul role="menu" aria-label="Account navigation" className="m-0 flex w-full list-none flex-col gap-1 py-1">
        <ProfileMenuItem href={APP_ROUTES.profile} icon="person" label="Profile" isDark={isDark} highlighted onClick={onClose} />
        <ProfileMenuItem href={APP_ROUTES.dashboard} icon="dashboard" label="Dashboard" isDark={isDark} onClick={onClose} />
        <ProfileMenuItem href={APP_ROUTES.settings} icon="settings" label="Settings" isDark={isDark} onClick={onClose} />
        <ProfileMenuItem href={APP_ROUTES.documentation} icon="help" label="Help Center" isDark={isDark} onClick={onClose} />
      </ul>
