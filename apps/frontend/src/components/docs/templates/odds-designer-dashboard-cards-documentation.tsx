  ...
  return (
    <div className="doc-example-container">
      <header className="doc-hero">
        <div className="doc-hero-badge">ODDS / Card Component</div>
        <h1 className="doc-hero-title">{card.name}</h1>
        <p className="doc-hero-subtitle">{card.usage}</p>
      </header>

      <section className="doc-section">
        <div className="doc-section-header">
          <h2 className="doc-section-title">Playground</h2>
        </div>
        <div className="mt-4">
      ...
      <section className="doc-section">
        ...
        <div className="doc-install-card mt-4">
          ...
          <div className="doc-install-body text-cu-muted">
            <code>apps/frontend/src/components/odds/{card.id}.tsx</code> (Conceptual path)
          </div>
        </div>
      </section>
          ...
          <table className="doc-table">
            <tbody>
              <tr>
                ...
                <td>{card.runtime === "rendered" ? "Rendered in dashboard" : "Hidden in dashboard, available in library guide"}</td>
              </tr>
              <tr>
                <th>Source</th>
                <td><code>{card.source}</code></td>
              </tr>
              <tr>
                <th>Implementation</th>
                <td>{card.implementation}</td>
              </tr>
            </tbody>
          </table>
        </div>
...
export function OddsDesignerDashboardCardsDocumentation({ cardId }: { cardId?: OddsDesignerDashboardCardId }) {
  ...
  return (
    <div className="doc-example-container">
      <header className="doc-hero">
        <div className="doc-hero-badge">ODDS / Dashboard</div>
        <h1 className="doc-hero-title">ODDS Card Component</h1>
        <p className="doc-hero-subtitle">
          Library guide seluruh card Dashboard Designer ODDS. Pilih card di bawah menu ini untuk melihat playground dan detail per card, termasuk card yang tidak di-render di runtime.
        </p>
      </header>

      <section className="doc-section">
        <div className="doc-section-header">
          <h2 className="doc-section-title">Card List</h2>
        </div>
        <div className="doc-grid mt-4">
          {cardDocs.map((card) => (
            <a key={card.id} href={`?section=components/odds-designer-dashboard-cards-${card.id}`} className="doc-grid-item" style={{ textDecoration: 'none', display: 'block', padding: '1.5rem', transition: 'all 0.2s ease' }}>
              <div className="flex items-start justify-between gap-3">
                <h3 style={{ margin: '0 0 0.5rem', fontSize: '1rem', fontWeight: 700, color: 'hsl(var(--foreground))' }}>{card.name}</h3>
                <span className={`shrink-0 rounded-full px-2 py-0.5 text-[10px] font-semibold uppercase tracking-[0.08em] ${card.runtime === "rendered" ? "bg-[#eef4ff] text-[#0b5cff]" : "bg-slate-100 text-slate-500"}`}>
                  {card.runtime}
                </span>
              </div>
              <p style={{ margin: 0, fontSize: '0.875rem', color: 'hsl(var(--muted-foreground))', lineHeight: 1.5 }}>{card.usage}</p>
            </a>
          ))}
        </div>
      </section>
      ...
      <section className="doc-section">
        <div className="doc-section-header">
          <h2 className="doc-section-title">Implementation Contract</h2>
        </div>
        <div className="doc-install-card mt-4">
          <div className="doc-install-body">
            Source render utama berada di <code>apps/frontend/src/app/odds/page.tsx</code>.<br />
            Title halaman memakai komponen global <code>HeaderTitle</code> dengan alignment kiri.
          </div>
        </div>
      </section>
    </div>
