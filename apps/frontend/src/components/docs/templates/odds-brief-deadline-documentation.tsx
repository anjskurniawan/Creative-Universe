...
import { BriefDeadlineStep } from "@/features/odds/components/request-builder/steps/brief-deadline-step";
...
function MockBriefDeadlineWrapper() {
  ...
  const [form, setForm] = useState<OddsRequestForm>({
    ...
    brief_text: "", reference_visual: "", deadline: "", important_matrix: "Q1", attachment_notes: "",
  });

  return (
    <div className="w-full flex flex-col items-center gap-4">
      <div className="flex gap-2 items-center">
        <span className="text-sm font-semibold">Simulate Matrix Level:</span>
        ...
        <button onClick={() => setForm(prev => ({...prev, important_matrix: "Q3"}))} className={`px-4 py-2 text-xs font-bold rounded-lg border-2 border-[#04044A] ${form.important_matrix === "Q3" ? 'bg-[#00E5FF] text-[#04044A]' : 'bg-white'}`}>Q3/Q4</button>
      </div>
      <div className="w-full max-w-[850px] bg-white border border-slate-200 rounded-3xl shadow-sm flex flex-col p-8 min-h-[300px]">
        <BriefDeadlineStep 
          form={form} 
          update={(field, value) => setForm(prev => ({...prev, [field]: value}))}
...
export function OddsBriefDeadlineDocumentation() {
...
import { BriefDeadlineStep } from "@/features/odds/components/request-builder/steps/brief-deadline-step";
export default function Page() { return <BriefDeadlineStep {...props} />; }`;

  return (
    <div className="doc-example-container">
      <header className="doc-hero">
        <div className="doc-hero-badge">ODDS / Request Builder</div>
        <h1 className="doc-hero-title">BriefDeadlineStep</h1>
        <p className="doc-hero-subtitle">Interactive documentation for the Request Deadline step.</p>
      ...
      <section className="doc-section">
        <div className="doc-section-header">
          ...
          <p className="doc-section-desc">Live preview of BriefDeadlineStep.</p>
        </div>
      ...
      <section className="doc-section">
        ...
        <div className="doc-install-card mt-4">
          ...
          <div className="doc-install-body text-cu-muted"><code>src/features/odds/components/request-builder/steps/brief-deadline-step.tsx</code></div>
        </div>
      </section>
    </div>
  );
