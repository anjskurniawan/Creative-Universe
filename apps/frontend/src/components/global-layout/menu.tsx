...
export default function Menu({
  ...
  useEffect(() => {
    ...
    const nestedActiveIndex = menuItems
      ...
      .sort((left, right) => (right.item.href?.length ?? 0) - (left.item.href?.length ?? 0))[0]?.index;
    const activeIndex = Math.max(0, exactActiveIndex >= 0 ? exactActiveIndex : (nestedActiveIndex ?? 0));
