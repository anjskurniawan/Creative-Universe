...
import { calculateHrdScore } from "./assessment-table.utils";
import { AssessmentTableAvatar } from "./assessment-table-avatar";
import { AssessmentTableCell } from "./assessment-table-cell";
...
export function AssessmentTableRow({ item, rowIndex, month, draft, inputMode, scoreMaxima, headers, hovered, onHover, updateDraft, addDate, setActiveDateAction }: { item: Assessment; rowIndex: number; month: string; draft: Draft; inputMode: boolean; scoreMaxima: number[]; headers: AssessmentHeader[]; hovered: boolean; onHover: (id: number | null) => void; updateDraft: (id: number, field: "score", value: string, scoreIndex?: number) => void; addDate: (id: number, key: HrdDateKey, dateStr: string) => void; setActiveDateAction: (action: ActiveDateAction) => void }) {
  ...
  return (
    <tr className={finalScore < 75 ? "bg-[#ffedf1] hover:bg-[#fff0f3]" : "bg-white hover:bg-[#fbfcfd]"}>
