...
export default function Card({
  ...
  useEffect(() => {
    if (!videoMedia || !autoPlayMedia || !mediaRef.current) return;
    void mediaRef.current.play();
  }, [autoPlayMedia, videoMedia, cardImage]);

  return (
    <article
      className={`cu-style relative flex w-full min-w-0 flex-col items-start gap-2 overflow-hidden rounded-lg bg-white p-2 text-[#3b4446] shadow-[0_5px_14px_rgba(44,42,39,0.06)] ${compact ? "" : "lg:min-w-[360px] lg:p-4"} ${onClick ? "cursor-pointer transition-[box-shadow,outline-color,transform] duration-300 ease-out will-change-[box-shadow,transform] hover:-translate-y-0.5 hover:shadow-[0_0_0_1px_rgba(0,164,255,0.28),0_0_18px_rgba(0,164,255,0.28)]" : ""} ${active ? "outline outline-2 outline-[#00a4ff] shadow-[0_0_18px_rgba(0,164,255,0.42)]" : ""} ${className}`}
      onClick={onClick}
