...
export default function Navbar({
  ...
  const [messages, setMessages] = useState<
    Array<{ id: string; sender: string; preview: string; time: string; unread?: boolean }>
  >([]);
  
  const [notifications, setNotifications] = useState<
    Array<{ id: string; title: string; content: string; time: string; read?: boolean; icon?: string }>
  >([]);
  ...
  return (
    ...
    >
      {showNavigation &&
        (viewport === "Mobile" ? (
          <button type="button" onClick={onMenuClick} className="flex size-10 items-center justify-center" aria-label="Menu">
      ...
      <div className="ml-auto flex h-full items-center gap-1">
        ...
        <div className="relative flex h-full items-center">
          ...
          {openMenu === "notifications" && (
            <NotificationDropdown isOpen onClose={() => setOpenMenu(null)} notifications={notifications} />
          )}
        </div>
        ...
        <div className="relative flex h-full items-center">
          ...
          {openMenu === "messages" && (
            <MessageDropdown isOpen onClose={() => setOpenMenu(null)} messages={messages} />
          )}
        </div>
