  ...
  return (
    ...
    >
        ...
        {menuItems.map((item, index) => (
          <li key={item.label}>
            <Link
              href={item.href}
              onClick={onClose}
              className={`flex h-10 items-center gap-2.5 rounded-xl px-3 text-sm text-slate-800 transition-colors hover:bg-slate-50 ${index === 0 ? "bg-slate-50" : ""}`}
            >
      ...
      <button
        ...
        className="flex h-10 w-full items-center gap-2.5 rounded-xl px-3 text-left text-sm text-slate-800 transition-colors hover:bg-slate-50"
      >
