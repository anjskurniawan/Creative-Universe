...
export function TaskCardWideDate({ quadrant, date, day, monthYear, time, isDone }: Pick<DateDisplayProps, "quadrant" | "date" | "day" | "monthYear" | "time" | "isDone">) {
  return <><div className={`flex px-1 shrink-0 items-center justify-center text-white ${isDone ? "bg-[#17633d]" : "bg-[#0077bf]"}`}><span className="text-[10px] font-medium tracking-[0.08em] [writing-mode:vertical-rl] rotate-180">{quadrant}</span></div><div className={`flex w-24 shrink-0 flex-col items-center justify-center text-white ${isDone ? "bg-[#238653]" : "bg-[#00a4ff]"}`}><span className="text-[10px] font-semibold">{isDone ? "SELESAI" : `${time} ${day}`}</span><span className="text-[42px] font-medium leading-none">{date}</span><span className="text-xs">{monthYear}</span></div></>;
}
