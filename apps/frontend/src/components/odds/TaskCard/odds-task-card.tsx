...
function taskStatusLabel(task: Pick<OddsTask, "status" | "task_type">) {
  ...
  if (task.task_type === "leader_revision") return task.status === "in_progress" ? "Pengerjaan Revisi SPV" : "Revisi SPV";
...
export function OddsTaskCard({
  ...
  const iconButton = ({ action, icon, label, active = false, danger = false, disabled = false }: { action: OddsTaskCardAction; icon: string; label: string; active?: boolean; danger?: boolean; disabled?: boolean }) => disabled ? null : (
  ...
  const actions = (mobile = false, fillHeight = false) => (
    <TaskCardActionBar mobile={mobile} fillHeight={fillHeight} overlay={actionOverlayOpen ? actionOverlay : undefined}>
      {iconButton({ action: "brief", icon: "description", label: "Detail brief", active: activeTab === "brief", disabled: isActionDisabled("brief") })}
      {chatAvailable && iconButton({ action: "chat", icon: "chat", label: "Diskusi task", active: chatOpen, disabled: isActionDisabled("chat") })}
      ...
      {(showStart || showPause || showDone || showAllActions) && iconButton({
        action: workAction,
        icon: workActionIcon,
