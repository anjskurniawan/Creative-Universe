...
function getMobileTaskTone(task: HomeworkTask): TaskcardMobileTone {
...
function requiredDelayReasonStage(task: HomeworkTask): string | undefined {
...
function getMobileTaskChanges(task: HomeworkTask): TaskcardMobileChange[] {
...
function getMobileCountdown(task: HomeworkTask) {
...
export function TaskPage({ scope = "all" }: { scope?: TaskPageScope }) {
  ...
  const activeTaskMenuLabel = scope === "unfinished"
    ...
    ? "Tugas Bulan Ini"
    : "Tugas Hari Ini";
  const isTaskAdministrator = Boolean(user?.roles.some((role) => ["Root", "Manajer", "SPV"].includes(role)));
  ...
  const handleMobileFileUpload = async (taskId: number, fileType: "support_file" | "draft_file", fileIndex: number, file: File) => {
    ...
    const savedTask = await apiFetch<HomeworkTask>(`/homework-tasks/${taskId}/upload`, {
      method: "POST",
      body: formData,
    });
    mergeTask(savedTask);
  ...
  const handleMobileViewFile = (file: string) => {
    ...
    window.open(file.startsWith("http://") || file.startsWith("https://") ? file : `${baseUrl}/storage/${file}`, "_blank");
  };
