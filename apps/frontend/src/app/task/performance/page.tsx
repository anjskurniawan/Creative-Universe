...
const TASK_NAVIGATION_ITEMS: TaskMobileNavigationItem[] = [
  { label: "Tugas Hari Ini", icon: "today", href: "/task" },
  ...
  { label: "Option Page", icon: "settings", href: "/task/option" },
];
...
const DESKTOP_SECONDARY_MENU: SideMenuItem[] = [
...
export default function TaskPerformancePage() {
  ...
  useEffect(() => {
    void apiFetch<TaskPerformanceTask[]>("/homework-tasks")
      .then(setTasks)
      .catch((error) => {
  ...
  useEffect(() => {
    if (user && !user.roles.some((role) => ["Root", "Manajer", "SPV"].includes(role))) {
  ...
  return (
    <>
      ...
      <div className="hidden min-h-screen grid-cols-[auto_minmax(0,1fr)] bg-[#f6faff] text-[#222] lg:grid">
        <SideMenu variant={desktopSidebarVariant} primaryItems={DESKTOP_PRIMARY_MENU} secondaryItems={DESKTOP_SECONDARY_MENU} onVariantChange={setDesktopSidebarVariant} />
