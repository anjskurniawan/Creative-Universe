...
export function DefaultPreviewPlaceholder({ componentName, reason }: { componentName: string; reason?: string }) {
  return (
    <div className="flex min-h-[250px] w-full flex-col items-center justify-center gap-2 rounded-2xl border border-dashed border-slate-100 bg-slate-50 p-6 text-slate-400">
      ...
      <span className="text-[10px] text-slate-400">{reason ?? "Pratinjau visual interaktif belum didefinisikan untuk komponen ini."}</span>
    </div>
  );
}
