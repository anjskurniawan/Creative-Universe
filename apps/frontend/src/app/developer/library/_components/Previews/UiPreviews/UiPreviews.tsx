...
import { SpinningWheel } from "@/components/ui/SpinningWheel/SpinningWheel";
...
type ActionCardExample = "report" | "team" | "settings";

const actionCardExamples: Record<ActionCardExample, { title: string; description: string; icon: string; href: string }> = {
  report: { title: "Unduh Laporan", description: "Ekspor statistik laporan ini ke berkas dokumen PDF secara instan.", icon: "download", href: "#" },
  team: { title: "Kelola Tim", description: "Atur anggota, peran, dan akses tim kreatif dari satu tempat.", icon: "groups", href: "#" },
  settings: { title: "Pengaturan Sistem", description: "Sesuaikan preferensi aplikasi dan konfigurasi workspace Anda.", icon: "settings", href: "#" },
};

export function ActionCardPreview({ example = "report" }: { example?: ActionCardExample }) {
  const selectedExample = actionCardExamples[example];
  return <div className={frame}><div className="w-64"><ActionCard {...selectedExample} /></div></div>;
}
