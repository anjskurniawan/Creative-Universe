...
import { RequestLaunchSequence } from "@/features/odds/components/OddsRequestBuilder/RequestLaunchSequence/RequestLaunchSequence";
import { useOddsTheme } from "@/features/odds/context/OddsThemeContext";
import {
  OddsRequestBuilder,
  type OddsRequestBuilderDraft,
} from "@/features/odds/components/OddsRequestBuilder";
import type { OddsRequestForm } from "@/features/odds/types";
import { matchesSpecialization, designerSort, recommendDesigner } from "@/features/odds/utils/designer";
import { stripRichText } from "@/features/odds/components/OddsRichTextEditor/OddsRichTextEditor";
import { useAuth } from "@/hooks/auth";
import { briefWithReferenceAliases, extractOddsBriefReferences } from "@/features/odds/utils/brief-references";
import {
  OddsCategory,
  OddsDesignerProfile,
  createOddsTask,
...
const emptyForm: OddsRequestForm = {
  request_type: "design",
  category_id: "",
...
export function NewOddsTaskPage() {
  ...
  const [form, setForm] = useState<OddsRequestForm>(emptyForm);
  ...
  const update = useCallback((field: keyof OddsRequestForm, value: string) => {
  ...
  return (
    <div ref={pageRef} className="cu-style relative min-h-0 w-full flex-1 flex flex-col p-0">
