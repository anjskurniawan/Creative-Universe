...
interface ApplicationUniverseProps {
 ...
 */
export function ApplicationUniverse({ applications, isReady }: ApplicationUniverseProps) {
  ...
  useEffect(() => {
    ...
    const context = gsap.context(() => {
      ...
      media.add("(prefers-reduced-motion: reduce)", () => {
        gsap.set(universeRef.current, { autoAlpha: 1, scale: 1 });
        gsap.set("[data-universe-core-glow], [data-universe-core], [data-universe-logo], [data-orbit-track], [data-planet]", { autoAlpha: 1, scale: 1 });
      });
      return () => media.revert();
    }, universeRef);
  ...
  return (
    <section aria-label="Aplikasi yang tersedia di Creative Universe" className="relative isolate flex min-h-[28rem] items-center justify-center overflow-hidden bg-white px-6 py-8 lg:min-h-0">
      ...
      {/* Informasi jika tidak ada sub-aplikasi yang terdaftar */}
      {applications.length === 0 && (
        <p className="absolute bottom-7 left-1/2 z-30 w-max max-w-[calc(100%-3rem)] -translate-x-1/2 text-center text-sm text-[#4b4646]">
          Belum ada sub-aplikasi aktif untuk akun ini.
        </p>
      )}
    </section>
  );
