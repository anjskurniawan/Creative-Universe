...
export default function AccountPrivacySettings() {
  ...
  return <form onSubmit={save} className="w-full space-y-5"><ToastContainer placement="bottom end" />{status && <InlineAlert variant="positive" fillStyle="subtleFill"><Heading>Berhasil</Heading><Content>{status}</Content></InlineAlert>}{error && <InlineAlert variant="negative" fillStyle="subtleFill"><Heading>Gagal menyimpan</Heading><Content>{error}</Content></InlineAlert>}<div className="rounded-2xl border border-cu-line bg-cu-surface p-5"><Switch size="XL" isSelected={showApplications} onChange={setShowApplications}>Tampilkan daftar aplikasi saya</Switch><p className="mt-2 text-sm text-cu-muted">Izinkan daftar aplikasi yang Anda akses terlihat pada profil pengguna lain.</p></div><Button type="submit" variant="primary" size="XL" isDisabled={saving} isPending={saving}><Text>{saving ? "Menyimpan..." : "Simpan Preferensi"}</Text></Button></form>;
}
