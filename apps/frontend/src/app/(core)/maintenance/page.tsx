...
export default function MaintenancePage() {
  ...
  useEffect(() => {
    ...
    apiFetch<{ active: boolean }>("/maintenance/emergency")
      .then((response) => setEmergencyActive(response.active))
      .catch((err: unknown) => setError(err instanceof Error ? err.message : "Gagal mengambil status maintenance darurat."))
  ...
  const updateEmergencyMode = async (active: boolean) => {
    ...
    try {
      ...
      setNotice(response.active ? "Maintenance darurat aktif. Hanya Root yang dapat mengakses aplikasi." : "Maintenance darurat dinonaktifkan. Akses pengguna telah dipulihkan.");
    } catch (err) {
      setError(err instanceof Error ? err.message : "Gagal mengubah maintenance darurat.");
    } finally {
    ...
    <div className="space-y-6">
      <header>
        ...
        <p className="mt-1 text-sm text-cu-muted">
          Kelola status internal Laravel, cache, antrean queue, dan fondasi database hosting secara aman tanpa terminal SSH.
        </p>
      </header>
      ...
      <section className={`rounded-2xl border p-5 shadow-sm ${emergencyActive ? "border-cu-danger/30 bg-cu-danger-soft" : "border-cu-line bg-cu-surface"}`}>
        <div className="flex flex-col gap-5 md:flex-row md:items-center md:justify-between">
          <div className="flex gap-3">
            <span className={`flex size-11 shrink-0 items-center justify-center rounded-xl ${emergencyActive ? "bg-cu-danger text-white" : "bg-cu-panel-soft text-cu-muted"}`}>
            ...
            <div>
              ...
              <p className="mt-1 max-w-2xl text-sm leading-6 text-cu-muted">Saat aktif, hanya Root yang dapat menggunakan aplikasi. Semua pengguna lain melihat halaman error universal dan request API mereka ditolak oleh backend.</p>
            </div>
          </div>
          <button type="button" disabled={isEmergencyLoading || isEmergencySaving} onClick={() => void updateEmergencyMode(!emergencyActive)} className={`shrink-0 rounded-full px-5 py-3 text-sm font-bold transition disabled:cursor-not-allowed disabled:opacity-60 ${emergencyActive ? "bg-white text-cu-danger ring-1 ring-cu-danger/20" : "bg-cu-danger text-white"}`}>
            {isEmergencyLoading ? "Memuat status..." : isEmergencySaving ? "Menyimpan..." : emergencyActive ? "Nonaktifkan Darurat" : "Aktifkan Darurat"}
          </button>
        </div>
      </section>
        ...
        <section className="rounded-2xl border border-cu-line bg-cu-surface p-5 shadow-sm space-y-4">
          ...
          <div className="flex flex-col gap-2">
            ...
            <CommandButton
              ...
              icon="verified_user"
              loading={isExecuting && activeCommand === "seed-production"}
              disabled={isExecuting}
              onClick={() => {
                if (window.confirm("Jalankan seeder fondasi hosting? Seeder ini tidak menghapus data transaksi atau membuat data demo.")) {
                  void runCommand("seed-production", "Seed Hosting (Aman)");
                }
              }}
            />
            <CommandButton
              ...
              icon="restore_page"
              loading={isExecuting && activeCommand === "restore-hosting-data"}
              disabled={isExecuting}
              onClick={() => {
                if (window.confirm("Pulihkan user nyata dan KV Retail Task dari file backup privat? Data dengan identitas legacy yang sama akan diperbarui, bukan diduplikasi.")) {
                  void runCommand("restore-hosting-data", "Pulihkan Data Hosting");
                }
              }}
            />
            <CommandButton
...
function AccessDenied() {
  return (
    <div className="rounded-2xl border border-cu-danger/20 bg-cu-danger-soft p-8 text-center max-w-lg mx-auto mt-12">
      ...
      <p className="mt-1 text-sm text-cu-muted">
        Hanya Root yang dapat membuka halaman pemeliharaan sistem.
      </p>
    </div>
  );
}
