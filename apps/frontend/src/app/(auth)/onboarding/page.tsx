...
function OnboardingCard() {
  ...
  // Display labels for custom dropdown
  ...
  return (
    <div className="flex flex-col items-center w-full z-10 relative">
      <div className="bg-white border border-[#edeef0] md:rounded-[16px] rounded-t-[32px] shadow-[0px_8px_12px_rgba(0,0,0,0.15)] flex flex-col items-start w-full md:max-w-[488px] h-[75vh] md:h-[612px] overflow-hidden relative transition-all duration-300">
        
        {/* Card Header */}
        <div className="bg-white border-[#edeef0] border-b border-solid flex items-center px-8 py-4 w-full shrink-0">
        ...
        {/* Global Error Banner */}
        {error && (
          <div className="absolute top-[60px] left-0 right-0 z-50 px-8 py-2">
            <div className="flex w-full rounded-[8px] bg-[rgba(255,56,60,0.14)] px-3 py-[10px] shadow-sm" role="alert">
          ...
          <div className={`w-full h-full flex flex-col items-center justify-center ${transitionClass}`}>
            ...
            {step === 3 && (
              <div className="flex flex-col items-center justify-center text-center w-full">
                <h2 className="font-sans font-semibold text-[#111827] text-[32px] leading-tight w-full mb-6">
              ...
              <div className="flex flex-col items-start w-full">
                ...
                <div className="flex flex-col gap-8 w-full relative">
                  <div className="flex flex-col gap-2 items-start w-full">
                    ...
                    <div 
                      onClick={() => !isLoadingData && setIsDivDropdownOpen(!isDivDropdownOpen)}
                      className={`border border-[#edeef0] flex h-[52px] items-center justify-between px-4 rounded-[8px] w-full cursor-pointer bg-white relative transition-all ${isLoadingData ? "opacity-70" : "hover:border-gray-300"}`}
                    >
                      <p className={`text-[16px] ${divisionLabel ? "text-gray-900" : "text-[#b9bdc7]"}`}>
                        {isLoadingData ? "Memuat..." : (divisionLabel || "Pilih divisi")}
                      </p>
                      <ChevronCircle />
                    </div>
                    {isDivDropdownOpen && !isLoadingData && (
                      <div className="absolute top-[84px] left-0 w-full bg-white border border-[#edeef0] rounded-[8px] shadow-lg z-50 max-h-[200px] overflow-y-auto animate-fade-in">
                        {divisions.map((div) => (
                          <div
                            key={div.id}
                            onClick={() => {
                              setDivisionId(div.id.toString());
                              setPositionId("");
                              setPositionName("");
                              setIsDivDropdownOpen(false);
                            }}
                            className="px-4 py-3 hover:bg-[#E7F3FF] hover:text-[#0088FF] text-[#232925] font-medium text-[16px] cursor-pointer transition-colors"
                          >
                            {div.name}
                          </div>
                        ))}
                  ...
                  <button
                    type="button"
                    onClick={() => divisionId && changeStep(6)}
                    disabled={!divisionId || isLoadingData}
                    className={`bg-[#286bff] text-white font-sans font-semibold text-[20px] rounded-[10px] w-full h-[56px] flex items-center justify-center shadow-[0px_4px_4px_rgba(59,130,246,0.2)] transition-all ${
                      divisionId && !isLoadingData ? "hover:bg-[#1a5bf0] active:bg-[#0c4ce0] cursor-pointer" : "opacity-50 cursor-not-allowed"
                    }`}
                  >
                    Lanjut
                  </button>
            ...
            {step === 6 && (
              <div className="flex flex-col items-start w-full">
                ...
                <div className="flex flex-col gap-8 w-full relative">
                  <div className="flex flex-col gap-2 items-start w-full">
                    ...
                    {isCreativeDivision ? (
                      <>
                        ...
                        >
                          <p className={`text-[16px] ${positionLabel ? "text-gray-900" : "text-[#b9bdc7]"}`}>
                            {positionLabel || "Pilih jabatan"}
                          </p>
                          <ChevronCircle />
                        </div>
                        {isPosDropdownOpen && (
                  ...
                  <button
                    type="button"
                    onClick={() => (isCreativeDivision ? positionId : positionName) && changeStep(7)}
                    disabled={isCreativeDivision ? !positionId : !positionName}
                    className={`bg-[#286bff] text-white font-sans font-semibold text-[20px] rounded-[10px] w-full h-[56px] flex items-center justify-center shadow-[0px_4px_4px_rgba(59,130,246,0.2)] transition-all ${
                      (isCreativeDivision ? positionId : positionName) ? "hover:bg-[#1a5bf0] active:bg-[#0c4ce0] cursor-pointer" : "opacity-50 cursor-not-allowed"
                    }`}
                  >
                    Lanjut
                  </button>
                  ...
                  <div className="flex flex-col gap-2 items-start w-full">
                    ...
                    <div className="border border-[#edeef0] flex h-[52px] items-center px-4 rounded-[8px] w-full focus-within:border-[#286bff] focus-within:ring-1 focus-within:ring-[#286bff] transition-all bg-white">
                      ...
                      <input
                        ...
                        className="w-full h-full text-[16px] text-gray-900 placeholder-[#b9bdc7] bg-transparent focus:outline-none disabled:opacity-70"
                      />
                    </div>
                  </div>
                  <button
                    ...
                    className={`bg-[#286bff] text-white font-sans font-semibold text-[20px] rounded-[10px] w-full h-[56px] flex items-center justify-center shadow-[0px_4px_4px_rgba(59,130,246,0.2)] transition-all ${
                      whatsapp && !isSubmitting ? "hover:bg-[#1a5bf0] active:bg-[#0c4ce0] cursor-pointer" : "opacity-50 cursor-not-allowed"
                    }`}
                  >
                    {isSubmitting ? (
        ...
        {/* Card Footer - Dynamic text */}
        <div className="bg-white border-t border-[#edeef0] flex items-center px-6 py-4 w-full shrink-0 min-h-[52px]">
          <p className="font-sans font-normal text-[12px] text-[#8a91a1] tracking-[0.6px] leading-[1.5] w-full">
            {step <= 3 && "Butuh bantuan? Buka Help Center"}
            {(step === 4 || step === 5 || step === 6 || step === 7) && "Bantu kami mengenali kamu"}
            {step === 8 && "Butuh bantuan? Buka Help Center"}
          </p>
        </div>
      </div>
    </div>
    ...
    <main className="relative min-h-screen overflow-hidden bg-cu-surface font-sans text-[#232925]">
      ...
      <div className="relative z-10 flex min-h-screen w-full items-end justify-center px-0 pt-10 md:items-center md:px-5 md:py-10">
        <div ref={cardRef} className="w-full flex justify-center z-10">
          <Suspense fallback={<div className="relative z-10 min-h-[368px] w-full rounded-t-[32px] bg-white px-8 pb-[20vh] pt-8 shadow-2xl md:max-w-[430px] md:rounded-[28px] md:px-9 md:py-10" />}>
            <OnboardingCard />
          </Suspense>
        </div>
        ...
        <div className="pointer-events-none absolute bottom-8 left-1/2 z-10 hidden w-full -translate-x-1/2 flex-col items-center md:flex">
          <div className="mb-6 flex items-center justify-center gap-4">
            ...
            <img src="https://doran.id/wp-content/uploads/2023/03/Logo-PT-Doran-Sukses-Indonesia-white-1400x364-1.png" alt="Doran Sukses Indonesia Logo" className="h-10 w-auto brightness-0 invert opacity-80" />
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src="https://jete.id/wp-content/uploads/2023/04/jete-indonesia-logo.png" alt="JETE Logo" className="h-10 w-auto brightness-0 invert opacity-80" />
          </div>
        </div>
      </div>
