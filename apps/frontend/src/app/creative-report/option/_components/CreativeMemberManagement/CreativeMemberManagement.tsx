...
export function CreativeMemberManagement({ section = "all" }: { section?: "pending" | "historical" | "all" }) {
  ...
  return <section className={`cu-style w-full ${section === "all" ? "grid gap-4 xl:grid-cols-2" : ""}`}>
    ...
    {showHistorical && <form onSubmit={addHistorical} className="w-full">
      ...
      <div className="mt-4 grid gap-2 sm:grid-cols-2"><input required value={historical.name} onChange={(event) => setHistorical({ ...historical, name: event.target.value })} placeholder="Nama personel" className="h-10 rounded-lg border border-[#dbe4e8] px-3 text-sm outline-none" /><select value={historical.position_name} onChange={(event) => setHistorical({ ...historical, position_name: event.target.value as HistoricalCreativeMemberInput["position_name"] })} className="h-10 rounded-lg border border-[#dbe4e8] px-3 text-sm"><option>SPV</option><option>Designer</option><option>Videographer</option><option>Content Creator</option></select><label className="text-xs text-[#7b868a]">Mulai<input required type="month" value={historical.start_month} onChange={(event) => setHistorical({ ...historical, start_month: event.target.value })} className="mt-1 block h-10 w-full rounded-lg border border-[#dbe4e8] px-3 text-sm text-[#3b4446]" /></label><label className="text-xs text-[#7b868a]">Sampai<input required type="month" value={historical.end_month} onChange={(event) => setHistorical({ ...historical, end_month: event.target.value })} className="mt-1 block h-10 w-full rounded-lg border border-[#dbe4e8] px-3 text-sm text-[#3b4446]" /></label></div>
