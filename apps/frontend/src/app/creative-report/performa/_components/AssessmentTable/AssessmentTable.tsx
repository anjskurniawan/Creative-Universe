...
import type { CreativeReportSettings } from "@/features/creative-report/settings";
import { HrdDateModal, type HrdDateKey, type ActiveDateAction } from "../HrdDateModal/HrdDateModal";
import type { AssessmentTableProps, Draft } from "./AssessmentTable.types";
import { formatDateShort } from "./AssessmentTable.utils";
import { AssessmentTableHeader } from "./AssessmentTableHeader/AssessmentTableHeader";
import { AssessmentTableActions } from "./AssessmentTableActions/AssessmentTableActions";
import { AssessmentTableRow } from "./AssessmentTableRow/AssessmentTableRow";

export type { Draft } from "./AssessmentTable.types";
export { calculateHrdScore } from "./AssessmentTable.utils";
...
export function AssessmentTable({
  ...
  const reorder = async (targetId: number) => {
    ...
    setReordering(true);
    try { await onReorder(group.id, ids); } finally { setReordering(false); setDraggedId(null); }
  };

  return (
    <>
      <div className="cu-style hidden w-full overflow-hidden rounded-b-xl border border-t-0 border-[#c9bbfc] bg-white lg:block">
        <table className="w-full table-fixed border-collapse text-left">
          ...
          <tbody className="text-xs text-[#3b4446]">
            {group.assessments.map((item, rowIndex) => {
              ...
              return <AssessmentTableRow key={item.id} item={item} rowIndex={rowIndex} month={month} draft={draft} inputMode={inputMode} scoreMaxima={scoreMaxima} headers={headers} hovered={hoveredAssessmentId === item.id} onHover={setHoveredAssessmentId} updateDraft={updateDraft} addDate={addDate} setActiveDateAction={setActiveDateAction} draggable={canEdit && !reordering} onDragStart={() => setDraggedId(item.user.id)} onDragOver={(event) => event.preventDefault()} onDrop={() => void reorder(item.user.id)} />;
            })}
          </tbody>
