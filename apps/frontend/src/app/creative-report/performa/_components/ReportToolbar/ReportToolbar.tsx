...
import { CreativeReportMetricCard, type CreativeReportMetric } from "./CreativeReportMetricCard/CreativeReportMetricCard";
...
export function ReportToolbar({ search, onSearchChange, jobdesk, onJobdeskChange, jobdesks, metrics, showMetrics = true }: { search: string; onSearchChange: (value: string) => void; jobdesk: string; onJobdeskChange: (value: string) => void; jobdesks: string[]; metrics: ReportMetric[]; showMetrics?: boolean }) {
  ...
  return <section className={`cu-style mt-0 grid grid-cols-[minmax(0,1fr)_3rem] gap-3 sm:grid-cols-2 ${showMetrics ? "lg:grid-cols-[minmax(220px,1.2fr)_180px_repeat(3,minmax(170px,1fr))]" : "lg:grid-cols-[minmax(220px,1.2fr)_180px]"}`}>
