...
import { type TaskCardConfig } from "@/features/kv-retail/components/TaskCard";
...
export default function OptionPage() {
  ...
  return (
    <>
      ...
      <div className={`cu-style hidden h-screen w-screen overflow-hidden lg:block ${desktopTheme === "dark" ? "bg-[#111413]" : desktopTheme === "retro" ? "bg-[#dfe2d3]" : "bg-[radial-gradient(circle_at_8%_6%,#00e7ef_0,transparent_25%),radial-gradient(circle_at_95%_90%,#00a4ff_0,transparent_31%),linear-gradient(135deg,#00a4ff_0%,#000675_44%,#04044a_100%)]"}`}>
      <Container
        viewport="Desktop"
        contentProps={{
        ...
        menuItems={PRIMARY_MENU.map((item) => ({ label: item.label, icon: item.icon, href: item.href, isActive: item.href === "/kv-retail/option" }))}
        activeMenuHref="/kv-retail/option"
        menuTitle="KV Retail"
      >
      <TaskDesktopPageTransition className="cu-style relative min-w-0 flex min-h-0 flex-1 flex-col overflow-y-auto">
