...
import { type TaskPerformanceTask } from "@/features/kv-retail/components/TaskPerformance/TaskPerformance.types";
...
import { PerformanceContentTitle } from "@/app/kv-retail/performance/_components/PerformanceContentTitle/PerformanceContentTitle";
import { PerformanceMetricCard } from "@/app/kv-retail/performance/_components/PerformanceMetricCard/PerformanceMetricCard";
import { KV_RETAIL_PERFORMANCE_PAGE } from "@/features/kv-retail/performance-page-config";
import { PerformanceSideSummary } from "@/app/kv-retail/performance/_components/PerformanceSideSummary/PerformanceSideSummary";
import { PerformanceChartIndicators } from "@/app/kv-retail/performance/_components/PerformanceChartIndicators/PerformanceChartIndicators";
...
export default function TaskPerformancePage() {
  ...
  return (
    <>
      ...
      <div className={`hidden h-screen w-screen overflow-hidden lg:block ${performanceTheme === "dark" ? "bg-[#111413]" : performanceTheme === "retro" ? "bg-[#dfe2d3]" : "bg-[radial-gradient(circle_at_8%_6%,#00e7ef_0,transparent_25%),radial-gradient(circle_at_95%_90%,#00a4ff_0,transparent_31%),linear-gradient(135deg,#00a4ff_0%,#000675_44%,#04044a_100%)]"}`}>
      <Container
        viewport="Desktop"
        contentProps={{
          className: `flex h-full w-full flex-col overflow-hidden rounded-[16px] shadow-[0px_14px_42px_0px_rgba(44,42,39,0.16)] ${performanceTheme === "dark" ? "bg-[#111413] text-white" : performanceTheme === "retro" ? "bg-[#c9ccc0] font-mono" : "bg-[#f3fbff]"}`,
          sidebarTheme: performanceTheme,
          sidebarExpanded: performanceSidebarExpanded,
          onToggleSidebarExpanded: togglePerformanceSidebarExpanded,
          onToggleSidebarTheme: () => setPerformanceTheme((theme) => theme === "dark" ? "light" : "dark"),
          onToggleSidebarRetro: () => setPerformanceTheme((theme) => theme === "retro" ? "light" : "retro"),
          contentProps: { className: "flex h-full min-h-0 w-full flex-1 flex-col overflow-hidden p-4" },
        }}
        menuItems={TASK_NAVIGATION_ITEMS.map((item) => ({ label: item.label, icon: item.icon, href: item.href, isActive: item.href === "/kv-retail/performance" }))}
        activeMenuHref="/kv-retail/performance"
        menuTitle="KV Retail"
      >
