...
export default function PricetagGeneratorPage() {
  ...
  const notify = useCallback((message: string) => {
    pushLocalNotification(message, "/generator/pricetag", user?.id);
  }, [user]);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      try {
        const res = await pricetagApi.batches.detail(activeBatch.id);
        setActiveBatch(res);
      } catch (err) {
        console.error(err);
      }
  ...
  useEffect(() => {
    ...
    if (productIdParam) {
      ...
      if (!isNaN(prodId)) {
        ...
        pricetagApi.products.detail(prodId)
          .then((prod) => {
            setActiveTab("single");
  ...
  const handleGenerateSingle = async (e: FormEvent) => {
    ...
    try {
      const res = await pricetagApi.generations.single(selectedProduct.id, Number(wizardDiscountPrice));
  ...
  const loadChecklistProducts = useCallback(async () => {
    ...
    try {
      ...
      const firstPage = await pricetagApi.products.list(`?${params}`);
      ...
      if (!isChecklistDesktop && firstPage.meta.last_page > 1) {
        const remainingPages = await Promise.all(
          Array.from({ length: firstPage.meta.last_page - 1 }, (_, index) => {
            ...
            return pricetagApi.products.list(`?${nextParams}`);
          })
        );
        setChecklistProducts([
          ...firstPage.data,
  ...
  const handleChecklistGenerate = async (e: FormEvent) => {
    ...
    try {
      const res = await pricetagApi.generations.checklist({
  ...
  const handleBulkGenerate = async (e: FormEvent) => {
    ...
    try {
      const res = await pricetagApi.generations.csv(formData);
  ...
  const handleDownloadTemplate = async (e: React.MouseEvent) => {
    e.preventDefault();
    try {
      const res = await pricetagApi.products.list("?per_page=2000");
