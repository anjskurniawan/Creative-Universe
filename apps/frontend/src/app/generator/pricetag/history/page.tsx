...
export default function PricetagHistoryPage() {
  ...
  const notify = useCallback((message: string) => {
    pushLocalNotification(message, "/generator/pricetag/history", user?.id);
  }, [user]);

  const loadBatchDetail = useCallback(async (id: number, silent = false) => {
    if (!silent) setIsLoadingDetail(true);
    try {
      const res = await pricetagApi.batches.detail(id);
      setDetailBatch(res);
    } catch (err) {
  ...
  const loadBatches = useCallback(async (silent = false) => {
    ...
    try {
      ...
      const res = await pricetagApi.batches.list(`?page=${page}&per_page=${limit}`);
      setBatches(res.data);
  ...
  const downloadBatch = async (batch: PricetagBatch) => {
    setDownloadingBatchId(batch.id);
    try {
      await downloadPricetagBatch(batch);
    } catch (error) {
      notify(pricetagError(error));
    } finally {
