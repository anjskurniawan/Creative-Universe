...
import { errorMessage, ManagedRole } from "@/lib/admin";
...
export default function RolesPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [roleData, permissionData] = await Promise.all([
        apiFetch<ManagedRole[]>("/roles"),
        apiFetch<string[]>("/permissions"),
      ]);
      setRoles(roleData);
  ...
  const saveRole = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (editing) {
        await apiFetch(`/roles/${editing.id}`, {
          method: "PATCH",
    ...
    <div className="space-y-6">
      <header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <h1 className="mt-1 text-2xl font-semibold text-cu-ink">Role & Permission</h1>
              ...
              <section>
                ...
                <div className="grid max-h-72 gap-2 overflow-y-auto rounded-xl border border-cu-line bg-cu-panel-soft p-3 sm:grid-cols-2">
                  {permissions.map((permission) => (
                    <label key={permission} className="flex cursor-pointer items-center gap-3 rounded-lg border border-cu-line bg-cu-surface px-3 py-2 text-sm text-cu-ink">
