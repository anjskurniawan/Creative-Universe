            ...
            <Panel title="Brief Request" icon="edit_note">
              <div className="grid gap-4 md:grid-cols-2">
                ...
                <label className="block md:col-span-2">
                  <span className="mb-1.5 block text-sm font-medium text-cu-ink">Brief</span>
                  <textarea
                    value={form.brief_text}
                    onChange={(event) => update("brief_text", event.target.value)}
                    required
                    rows={7}
              ...
              <StepActions>
                ...
                <button
                  type="button"
                  disabled={!form.design_purpose.trim() || !form.brief_text.trim()}
          ...
          {activeStep === "validation" && (
            <Panel title="Validasi Submit" icon="verified">
              <div className="grid gap-3 md:grid-cols-2">
                ...
                />
                <ValidationCard
                  ok={Boolean(form.brief_text.trim())}
                  title="Brief"
                  description={form.brief_text.trim() ? "Brief siap direview designer." : "Brief wajib diisi."}
                />
              </div>
...
function isStepComplete(step: RequestStep, form: TaskForm): boolean {
  ...
  if (step === "brief") return Boolean(form.design_purpose.trim()) && Boolean(form.brief_text.trim());
