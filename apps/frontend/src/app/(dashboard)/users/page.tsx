...
export default function UsersPage() {
  ...
  const loadOptions = useCallback(async () => {
    try {
      const result = await apiFetch<UserManagementOptions>("/users/options");
      setOptions(result);
  ...
  const openUser = async (user: ManagedUser) => {
    ...
    try {
      const detail = await apiFetch<ManagedUserDetail>(`/users/${user.id}`);
      setSelected(detail);
      setForm({
  ...
  const saveUser = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      await apiFetch<ManagedUser>(`/users/${selected.user.id}`, {
        method: "PATCH",
        body: JSON.stringify({
          ...form,
  ...
  const revokeSession = async (sessionId: string) => {
    ...
    setModalError(null);
    try {
      await apiFetch(`/users/${selected.user.id}/sessions/${sessionId}`, {
        method: "DELETE",
      });
      setSelected({
        ...selected,
  ...
  const saveWhitelist = async () => {
    ...
    setError(null);
    try {
      await apiFetch<string[]>("/users/whitelist-manager-permissions", {
        method: "POST",
        body: JSON.stringify({ permissions: whitelist }),
      });
                  ...
                  <tr key={managedUser.id} className="transition hover:bg-cu-panel-soft/30">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex max-w-sm flex-wrap gap-1.5">
                        ...
                        {managedUser.permissions.map((permission) => <Badge key={permission} tone="soft">+{permission}</Badge>)}
                      </div>
                    </td>
            ...
            <form onSubmit={saveUser}>
              <div className={`grid gap-6 p-6 ${selected.can_view_audit ? "lg:grid-cols-[1.1fr_0.9fr]" : ""}`}>
                <div className="space-y-6">
                  ...
                  <CheckboxGroup title="Izin Langsung" items={options?.permissions || []} selected={form.permissions} onToggle={(value) => toggleArrayValue("permissions", value)} empty="Tidak ada permission yang dapat Anda delegasikan." />
                </div>
...
function CheckboxGroup({ title, items, selected, onToggle, empty }: { title: string; items: string[]; selected: string[]; onToggle: (value: string) => void; empty?: string }) {
  return (
