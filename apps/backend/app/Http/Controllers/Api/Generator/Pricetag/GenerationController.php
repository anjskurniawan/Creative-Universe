...
use App\Http\Requests\Generator\Pricetag\ChecklistPricetagGenerationRequest;
use App\Http\Requests\Generator\Pricetag\CsvPricetagGenerationRequest;
use App\Http\Requests\Generator\Pricetag\SinglePricetagGenerationRequest;
use App\Http\Resources\Generator\Pricetag\PricetagBatchResource;
use App\Http\Resources\Generator\Pricetag\PricetagProductResource;
...
use App\SubApps\Generator\Pricetag\Services\PricetagGeneratorService;
...
class GenerationController extends BaseApiController
{
     ...
     */
    public function downloadZip(PricetagBatch $batch, Request $request, FileStorageService $files)
    {
        $user = $request->user();
        ...
        if (! class_exists('ZipArchive')) {
            return $this->sendError('Ekstensi PHP zip (ZipArchive) tidak aktif di server ini.', [], 500);
        }

        $batch->load('items.product.assetLinks');

        if ($batch->status !== 'completed' && $batch->processed_items === 0) {
            return $this->sendError('Batch ini belum selesai diproses atau tidak memiliki item sukses.', [], 400);
        }

        $downloadName = 'pricetag-batch-'.Str::slug($batch->batch_name).'-'.$batch->id.'.zip';
        $temporaryFile = $files->temporaryLocalPath('generator', 'pricetag-batches', $batch->id, 'archives', 'zip');
        $zipPath = $temporaryFile['absolute_path'];
        ...
        if ($zip->open($zipPath, \ZipArchive::CREATE | \ZipArchive::OVERWRITE) === true) {
            ...
            if ($hasFiles) {
                ...
                return $this->sendError('Gagal mengunduh gambar untuk semua item di kelompok ini.', [], 400);
            }
        } else {
            return $this->sendError('Gagal membuat file ZIP di server.', [], 500);
        }
    }
}
