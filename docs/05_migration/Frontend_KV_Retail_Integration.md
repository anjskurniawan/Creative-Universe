...
Seluruh request domain KV Retail menggunakan `features/kv-retail/api` dan DTO `KvRetailTask`. Upload sementara kini membaca envelope backend `data.path` dengan benar. Pemilihan assignee menampilkan seluruh pengguna yang memiliki akses aplikasi KV Retail, tanpa pengecualian berdasarkan nama role.
